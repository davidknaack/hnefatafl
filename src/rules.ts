...
function getEdgeEnclosureCaptures(
  ...
  function scanLine(coords: Coordinate[], getInward: (c: Coordinate) => Coordinate) {
    ...
    function isNewlyEnclosed(segment: Coordinate[], getInward: (c: Coordinate) => Coordinate, start: number, end: number): boolean {
      ...
      const enclosedAfter = adjList.every(a => isHostileTo(opponentOwner, a.x, a.y));
      const enclosedBefore = adjList.every(a => isHostileToBefore(opponentOwner, a.x, a.y));
