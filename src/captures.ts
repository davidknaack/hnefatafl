import { Coordinate, Move, PieceType, Player, Square } from './types'
...
function getEdgeEnclosureCaptures(
    ...
    // Only physical edges form shieldwalls. Interior row/column groups contain
    // separated squares from opposite edges (and possibly escape points).
    for (const [y, coords] of byRow) {
        if (y === 0 || y === size - 1) {
            scanLine(coords, (c) => ({ x: c.x, y: c.y === 0 ? 1 : size - 2 }))
        }
    }
    for (const [x, coords] of byCol) {
        if (x === 0 || x === size - 1) {
